...
class TestRobotsLockSafety:
    """O1: robots dict에 대한 스레드 안전성 검증."""

    def test_concurrent_read_write_no_crash(self):
        """여러 스레드가 동시에 dict를 읽고 쓸 때 크래시 없음."""
        robots = {}
        lock = threading.Lock()
        errors = []
        ...
        def writer():
            for i in range(200):
                with lock:
                    robots[f'robot_{i}'] = RobotRecord(f'robot_{i}', last_seen=float(i))

        def reader():
            for _ in range(200):
                with lock:
                    snapshot = dict(robots)
                # snapshot 순회는 lock 밖에서 안전
        ...
        # 크래시 없이 완료되면 성공
        assert len(robots) == 200

    def test_snapshot_iteration_safe(self):
        """스냅샷 기반 순회 중 원본이 변경돼도 안전."""
        robots = {'a': RobotRecord('a'), 'b': RobotRecord('b')}
        snapshot = dict(robots)
        # 원본 변경
        robots['c'] = RobotRecord('c')
        del robots['a']
        # 스냅샷은 영향 없음
        assert list(snapshot.keys()) == ['a', 'b']


class TestReturnStartTimeGuard:
    """O2: return_start_time None 가드 검증."""

    def test_returning_stage_with_none_start_time(self):
        """RETURNING 단계에서 return_start_time=None이면 크래시 없이 처리."""
        # 시뮬레이션: return_start_time이 None인 상태에서 elapsed 계산
        return_start_time = None
        current_time_ns = 1000.0  # 현재 시각 (초)

        # 수정 전: TypeError 발생
        # elapsed = (current_time_ns - return_start_time)  # TypeError!

        # 수정 후: None 가드
        if return_start_time is None:
            return_start_time = current_time_ns
        elapsed = current_time_ns - return_start_time
        assert elapsed == 0.0  # 방금 시작한 것으로 처리

    def test_returning_stage_with_valid_start_time(self):
        """정상 return_start_time이 있으면 경과시간 정상 계산."""
        return_start_time = 100.0
